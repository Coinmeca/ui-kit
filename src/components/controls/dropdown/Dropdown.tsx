...
export default function Dropdown(props: Dropdown) {
	...
	const [openSelect, closeSelect] = usePortal(
		<BottomSheet height={{ max: "60vh" }}>
			<Layouts.Col style={{ padding: "2em" }} gap={2}>
				...
				<Controls.Button onClick={() => closeSelect()}>
					Close
				</Controls.Button>
			</Layouts.Col>
		</BottomSheet>
	...
	return (
		<Style
			...
			onClick={() =>
				!props?.responsive ? handleOpen() : openSelect(Select)
			}
			onBlur={handleClose}
			title={props?.title}
			data-active={open}
			...
			<Option>
				<Item>
					{form?.indexOf("more") === 0 ? (
						...
						<>
							{option &&
								typeof option[imgName] !== "undefined" && (
									<Image
										src={option[imgName]}
										width={0}
										height={0}
							...
							<span
								title={
									typeof option === "undefined"
										...
										? typeof option[keyIndex] !==
										  "undefined"
											? option[keyIndex]
											: typeof option[keyName] !==
											  "undefined"
											? option[keyName]
											: option
										: option
