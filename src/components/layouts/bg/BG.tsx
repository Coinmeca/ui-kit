...
export default function BG(props: BG) {
    ...
    const src =
        typeof props?.img?.src === "number"
            ? require(`../../../assets/pictures/${props?.img?.src}.jpg`)
            : props?.img?.src;
    const filter =
        (typeof props?.filter === "string" && props?.filter) ||
        (typeof props?.filter === "object" &&
            typeof props?.filter?.color === "string" &&
            props?.filter?.color) ||
        undefined;
    const opacity =
        (typeof props?.filter === "object" &&
            typeof props?.filter?.opacity === "string" &&
    ...
    return (
        <Style $fix={fix} style={{ background: props?.background }}>
            {props?.img?.src && (
                <Image src={src} style={props?.img?.style} fill alt={""} />
            )}
