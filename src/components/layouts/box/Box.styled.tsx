...
const Style = styled.div<{
    ...
    ${({ $padding, $fit }) => css`
        ...
        ${$fit
            ? `height: calc(100% - ${
              ...
              }em); min-height: max-content;`
            : `height: max-content; min-height: calc(100% - ${
                  $padding?.top + $padding?.bottom
              }em);`}
    `}
    ...
    ${Page.default} > ${SlideContainer.default} > * > & {
        @media all and (min-width: ${Root.Device.Desktop}px) {
            ${({ $padding, $fit }) => css`
                padding: ${$padding?.top}em ${$padding?.right * 2}em
                    ${$padding?.bottom}em ${$padding?.left * 2}em;
        ...
        @media all and (max-width: ${Root.Device.Mobile}px) {
            ...
            ${({ $padding, $fit }) => css`
                padding: ${$padding?.top / 2}em ${$padding?.right / 2}em
                    ${$padding?.bottom / 2}em ${$padding?.left / 2}em;
                width: calc(100% - ${($padding?.left + $padding?.right) / 2}em);
                ${$fit
                    ? `height: calc(100% - ${
