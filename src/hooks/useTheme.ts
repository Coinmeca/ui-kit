...
export default function useTheme(initial?: Mode): Theme {
    ...
    const [theme, setTheme] = useState<Mode>(!!initial ? initial : (detectForcedDarkMode() ? 'dark' : w?.matchMedia?.("(prefers-color-scheme: dark)") || CSS?.supports?.("color-scheme", "dark")) ? "dark" : "light");
