...
export default function Ordrebook(props: Orderbook) {
	...
	const ask_max: number =
		(asks &&
			asks?.length > 0 &&
			Math.max(
				...asks?.map((o: Tick) => parseFloat(o?.balance?.toString()))
			)) ||
		0;
	const bid_max: number =
		(bids &&
			bids?.length > 0 &&
			Math.max(
				...bids?.map((o: Tick) => parseFloat(o?.balance?.toString()))
			)) ||
		0;
	...
	const handleAsk = (ask: Tick, k: number, e?: any) => {
		if (typeof props?.onClickAsk === "function")
			props?.onClickAsk(ask, k, e);
	};

	const handleBid = (bid: Tick, k: number, e?: any) => {
		if (typeof props?.onClickBid === "function")
			props?.onClickBid(bid, k, e);
	};
		...
		const k = [...asks]
			...
			.reduce(
				(a: Tick, b: Tick) =>
					...
					parseFloat((b?.price || 0).toString()) *
						parseFloat((b?.balance || 0).toString()),
				0
			);
		const sum = [...asks]
			.splice(0, i + 1)
	...
	return (
		<Style $responsive={props?.responsive} $guidance={guidance}>
			...
			<Layouts.Divider
				responsive={props?.responsive?.device}
				style={{ ...(view !== 0 && { display: "none" }) }}
			/>
			<Bids
				$show={view === 0 || view === 2}
				onMouseLeave={() => closeTooltip()}
